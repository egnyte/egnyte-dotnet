...
namespace Egnyte.Api.Tests.ProjectFolders
{
    ...
    {
        const string GetAllProjectsTestsResponseContent = @"
           [
              {
        ...
        public async Task GetAllProjects_ReturnsSuccess()
        {
                ...
                (request, cancellationToken) =>
                    Task.FromResult(
                        new HttpResponseMessage
                        {
                            ...
                            Content = new StringContent(GetAllProjectsTestsResponseContent)
                        });

            var egnyteClient = new EgnyteClient("token", "acme", httpClient);
            var getAllProjectsResponse = await egnyteClient.ProjectFolders.GetAllProjects();
            var projectDetails = getAllProjectsResponse.First();
            ...
            Assert.AreEqual(1, getAllProjectsResponse.Count);
            Assert.AreEqual(1, getAllProjectsResponse.Count);
